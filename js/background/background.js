...
try {
  ...
  if (typeof Logger !== "undefined")
    Logger.info("Background Service Worker 啟動完成");
} catch (e) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "FETCH_CHANNEL_INFO") {
    ...
    const tabId = sender.tab ? sender.tab.id : "system";
    BgFetcher.fetch(request.handle, priority, forceRefresh, tabId).then(
      sendResponse
    );
    return true;
  }
